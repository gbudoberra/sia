...
def graph_util(data, err, x_label, y_label, title):
    ...
    for i, method in enumerate(labels):
        means = [data[j][method]  for j in range(len(dims))]
        errors = [err[j][method] for j in range(len(dims))]
...
if __name__ == '__main__':
    ...
    for i in dims:
        ...
        df = pd.read_csv('method_output_' + str(dim) + '.csv', delimiter=';')
    ...
    width = 0.2
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#aeaeff']


    graph_util(costs, costs_err, 'Dimensión', 'Costo (pasos)', 'Costo vs Dimensión por cada método')
    graph_util(n_nodes, n_nodes_err, 'Dimensión', 'Nodos generados', 'Nodos generados vs Dimensión por cada método')
    graph_util(n_border, n_border_err, 'Dimensión', 'Nodos frontera finales', 'Nodos frontera vs Dimensión por cada método')
